...
void CInstrumentRecorder::RecordInstrument(const unsigned Tick, CFamiTrackerView *pView)		// // //
{
	...
	switch (Chip) {
	case SNDCHIP_NONE:
		ID -= CHANID_2A03_SQUARE1;
		PitchReg = m_iRecordChannel == CHANID_2A03_NOISE ? (0x0F & REG(0x400E)) :
	...
	switch (Chip) {
		...
		case SNDCHIP_5B: case SNDCHIP_AY8930: case SNDCHIP_AY: case SNDCHIP_SSG: Table = CDetuneTable::DETUNE_S5B; break;
	}
	int Note = 0;
	...
	case SNDCHIP_5B:	case SNDCHIP_AY8930: case SNDCHIP_AY: case SNDCHIP_SSG: InstType = INST_S5B; break;
	}
	...
	case INST_2A03: case INST_VRC6: case INST_N163: case INST_S5B:
		for (int i = 0; i < SEQ_COUNT; i++) {
			...
			case SEQ_DUTYCYCLE:
				switch (Chip) {
				case SNDCHIP_NONE:
					Val = m_iRecordChannel == CHANID_2A03_TRIANGLE ? 0 :
						m_iRecordChannel == CHANID_2A03_NOISE ? (0x01 & REG(0x400E) >> 7) : (0x03 & REG(0x4000 | (ID << 2)) >> 6); break;
				case SNDCHIP_VRC6:
...
void CInstrumentRecorder::InitRecordInstrument()
{
	...
	*m_pDumpInstrument = CInstrumentFactory::CreateNew(Type);		// // //
	...
	case INST_2A03: case INST_VRC6: case INST_N163: case INST_S5B:
		...
		if (m_iRecordChannel == CHANID_2A03_TRIANGLE)
