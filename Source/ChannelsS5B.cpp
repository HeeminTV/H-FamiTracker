...
{
	WriteReg(8 + m_iChannelID - CHANID_5B_CH1, 0);		// Clear volume
}
...
void CChannelHandlerS5B::RefreshChannel()
{
	...
	WriteReg((m_iChannelID - CHANID_5B_CH1) * 2    , LoPeriod);
	WriteReg((m_iChannelID - CHANID_5B_CH1) * 2 + 1, HiPeriod);
	WriteReg((m_iChannelID - CHANID_5B_CH1) + 8    , Volume | Envelope);

	if (Envelope && (m_bTrigger || m_bUpdate))		// // // 050B
		s_bEnvTrigger = true;
	m_bUpdate = false;

	if (m_iChannelID == CHANID_5B_CH3)
		UpdateRegs();
}
